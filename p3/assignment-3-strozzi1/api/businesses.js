...
router.post('/', requireAuthentication, async (req, res) => {
  if (validateAgainstSchema(req.body, BusinessSchema)) { 
    try {
      const authorizedReview = await validateUserIdCombo(req.user, parseInt(req.body.ownerid) );
      ...
      res.status(500).send({
        error: "Unable to add business. Logged in user and ownerid need to be equal. Try again later."
      });
    } 
  } else {
    res.status(400).send({
...
router.put('/:id', requireAuthentication, async (req, res, next) => {
  if (validateAgainstSchema(req.body, BusinessSchema)) {
    ...
    try {
      ...
      if (business) {
        try {
          const authorizedBusiness = await validateUserIdCombo(req.user, business.ownerid );
          if( (!authorizedBusiness || req.body.ownerid !== business.ownerid) && !req.admin){
            res.status(403).send({
              error: "Unauthorized to access the specific resource"
            })
          } else {
          ...
          res.status(500).send({
            error: "Failure to validate owner of business to logged in user, try again later."
          });
...
router.delete('/:id', requireAuthentication ,async (req, res, next) => {
  ...
  try {
    ...
    if (business) {
      try {
        const authorizedReview = await validateUserIdCombo(req.user, business.ownerid );
        if(!authorizedReview && !req.admin){
          res.status(403).send({
            error: "Unauthorized to access the specific resource"
          })
        ...
        res.status(500).send({
          error: "Failure to validate userid of business to logged in user, try again later."
        });
      }
    } else {
      next();
